...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 比赛分数中心 <span
            class="c-gray en">&gt;</span> 比赛分数管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div class="text-c"> 日期范围：
        ...
        <button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜比赛分数
        </button>
    </div>
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="member_add('添加比赛分数','{{ route('matchscore_add') }}','','550')"
                    class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加比赛分数</a></span> <span
            ...
            @foreach($data as $item)
                @if($item->del == 1)
                    <tr class="text-c">
                        ...
                        <td>{{ $item->big_left }}</td>
                        <td>{{ $item->score_left }}</td>
                        <td>{{ $item->a_user }}</td>
                        <td>{{ $item->b_user }}</td>
                        <td>{{ $item->score_right }}</td>
                        <td>{{ $item->big_right }}</td>
                        <td>{{ $item->class }}</td>
                        <td class="td-manage">
                            <a title="编辑" href="javascript:;" onclick="member_edit('编辑','{{ route('matchscore_edit', ['id' => $item->id]) }}','4','','580')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
